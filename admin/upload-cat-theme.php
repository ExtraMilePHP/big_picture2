...
$cat_audio_theme = unserialize($data["cat_audio_theme"]);
...
$local_upload_path = '../themes/' . $new_filename; // Set the new upload path
...
if (move_uploaded_file($_FILES["file"]["tmp_name"], $local_upload_path)) {
    ...
    for ($i = 0; $i < sizeof($all_cat); $i++) {
        if ($all_cat[$i] == $selected_cat) {
            $cat_audio_theme[$selected_cat] = $new_filename;
        }
    }

    // Save updated category icons
    if (updateThemeData("cat_audio_theme", serialize($cat_audio_theme))) {
